...
- 📄 **PDF 自动提取文字**:拖入或 `@` 引用 PDF,自动转成文本喂给模型
...
| **拖拽文件/PDF 到窗口** | 自动作为附件,无需 `@` 前缀;**PDF 自动提取文字** |
...
| 工具 | 功能 | 需确认 |
|------|------|--------|
| `read_file` / `list_files` / `search_text` | 读文件(PDF 自动提取文字)/ 列文件 / 正则搜索 | ✗ |
...
**Q:能读 PDF 吗?**
A:可以。拖入或 `@` 引用 PDF 会自动提取其中的文字喂给模型(纯 JS 实现,无需额外安装)。扫描件(只有图片、没有文字层)无法提取,会给出明确提示。其它二进制文件会被识别并跳过,而不是塞乱码。
